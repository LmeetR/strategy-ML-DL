...
class SVM():
    ...
    def data_pre(self):
        ...
        # shift -1 将后一天的标签和前一天对齐，这样就可以预测了
        # 上涨标记为1，下跌标记为-1
    ...
    def gscv_para(self, C_list, gamma_list, x_train, y_train):
        ...
        # SVC（支持向量分类器）是一个流行的分类算法
        clf = SVC(class_weight='balanced', cache_size=4000)

        # GridSearchCV（简称 GSCV）用于通过网格搜索自动选择模型的最佳超参数
...
if __name__ == '__main__':
    ...
    plt.tight_layout()
    plt.savefig('attr_heatmap_latest.png')
    ...
    plt.savefig('v_svm_latest.png', bbox_inches='tight')
